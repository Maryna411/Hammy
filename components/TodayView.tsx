...
import { ListChecks, PartyPopper, ChevronDown } from "lucide-react";
  ...
  if (todayTasks.length === 0) {
    return (
      <div className="px-4 pt-6">
        <h1 className="mb-1 text-lg font-semibold text-white capitalize">{todayLabel}</h1>
        <EmptyState
          icon={<ListChecks size={24} />}
          title="План на сьогодні ще не сформований"
          description="Перейди в Inbox і натисни «Сформувати план на сьогодні»."
          action={
            <button
              onClick={onGoToInbox}
    ...
    <div className="px-4 pt-6 pb-4">
      ...
      {active.length === 0 ? (
        <div className="rounded-2xl border border-border bg-surface p-6 text-center">
          <PartyPopper className="mx-auto mb-2 text-accent2" size={24} />
          <p className="text-[15px] font-medium text-white">Усе на сьогодні зроблено 🎉</p>
        </div>
      ) : (
